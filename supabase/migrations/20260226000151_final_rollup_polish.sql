-- =============================================================================
-- Final polish: 3 low-severity audit fixes for rollup functions
-- =============================================================================
-- 1. Add group_id to review_intelligence ON CONFLICT UPDATE (consistency)
-- 2. Safe JSONB parsing in aggregate_item_mentions helper
-- 3. Clamp intensity to [1,5] before dividing by 5.0 in rollup_daily_flavor_index
-- =============================================================================

-- ─────────────────────────────────────────────────────────────────────────────
-- Fix 1 + 3: rollup_review_intelligence — add group_id to ON CONFLICT UPDATE
-- ─────────────────────────────────────────────────────────────────────────────
  ...
  LOOP
    ...
    LOOP
      ...
      ON CONFLICT (restaurant_id, period_type, period_start) DO UPDATE SET
        group_id             = EXCLUDED.group_id,           -- ← Fix 1: was missing
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Fix 3: rollup_daily_flavor_index — clamp intensity to [1,5] before /5.0
-- ─────────────────────────────────────────────────────────────────────────────
-- Prevents CHECK constraint violation on sentiment columns if AI outputs
-- intensity > 5. LEAST(5.0, GREATEST(1.0, ...)) ensures range stays valid.
...
BEGIN
  ...
  -- Score formula:
  --   strengths    →  +CLAMP(intensity, 1, 5) / 5.0  (range +0.2 to +1.0)
  --   opportunities → -CLAMP(intensity, 1, 5) / 5.0  (range -1.0 to -0.2)
  --
  -- NULLIF + regexp_replace guards against non-numeric intensity values.
  -- LEAST/GREATEST clamps to [1,5] to prevent CHECK constraint violations.
    ...
    CROSS JOIN LATERAL (
      SELECT
        ...
        +LEAST(5.0, GREATEST(1.0,
          NULLIF(regexp_replace(s.elem->>'intensity', '[^0-9.]', '', 'g'), '')::numeric
        )) / 5.0 AS score
      ...
      SELECT
        o.elem->>'category' AS category,
        -LEAST(5.0, GREATEST(1.0,
          NULLIF(regexp_replace(o.elem->>'intensity', '[^0-9.]', '', 'g'), '')::numeric
        )) / 5.0 AS score
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Fix 2: aggregate_item_mentions — safe JSONB intensity parsing
-- ─────────────────────────────────────────────────────────────────────────────
-- Original had raw (item->>'intensity')::numeric which crashes on non-numeric.
-- Now uses NULLIF + regexp_replace, consistent with rollup functions.
...
AS $$
  ...
  FROM (
    SELECT
      ...
      ROUND(AVG(
        NULLIF(regexp_replace(item->>'intensity', '[^0-9.]', '', 'g'), '')::numeric
      ), 1) AS avg_intensity
    FROM public.review_analyses ra,
