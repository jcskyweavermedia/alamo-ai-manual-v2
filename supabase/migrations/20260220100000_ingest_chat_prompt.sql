-- =============================================================================
-- MIGRATION: ingest_chat_prompt
-- Inserts the system prompt for the conversational recipe ingestion assistant.
-- Idempotent via ON CONFLICT (slug) DO UPDATE.
-- =============================================================================

INSERT INTO public.ai_prompts (
  slug, category, domain, prompt_en, prompt_es, is_active
)
VALUES (
  'ingest-chat-system',
  'system',
...
When calling `update_draft`, structure the data according to these fields:
...
```
{
  ...
  "ingredients": [
    {
      "group": "string — group name, e.g. Herb Base",
      "items": [
        {
  ...
  "procedure": [
    {
      "group": "string — phase name, e.g. Prep",
      "steps": [
        {
          "step_number": "number",
  ...
  "batchScaling": {
    ...
    "method": "string — e.g. linear, stepped, custom",
    "notes": "string — general scaling notes",
    "exceptions": "string — ingredients or steps that do not scale linearly"
  },
  "trainingNotes": {
    "notes": "string — key tips for cooks learning this recipe",
    "common_mistakes": "string — frequent errors to watch for",
    "quality_checks": "string — how to verify the recipe was executed correctly"
  }
}
```
...
- Usa listas con viñetas (`-`) para listar ingredientes, pasos u opciones.
...
Al llamar `update_draft`, estructura los datos de acuerdo a estos campos:
...
```
{
  ...
  "ingredients": [
    {
      "group": "string — nombre del grupo, ej. Base de Hierbas",
      "items": [
        {
  ...
  "procedure": [
    {
      "group": "string — nombre de la fase, ej. Preparacion",
      "steps": [
        {
          "step_number": "number",
  ...
  "batchScaling": {
    ...
    "method": "string — ej. linear, stepped, custom",
    "notes": "string — notas generales de escalado",
    "exceptions": "string — ingredientes o pasos que no escalan linealmente"
  },
  "trainingNotes": {
    "notes": "string — tips clave para cocineros aprendiendo esta receta",
    "common_mistakes": "string — errores frecuentes a vigilar",
    "quality_checks": "string — como verificar que la receta se ejecuto correctamente"
  }
}
```
