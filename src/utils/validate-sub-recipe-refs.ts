...
import type { RecipeIngredientGroup, PlateComponentGroup } from '@/types/products';
...
// -----------------------------------------------------------------------------
// Plate Spec Component Ref Validation
// -----------------------------------------------------------------------------

/**
 * Extract all unique `prep_recipe_ref` slugs from plate spec component groups
 * and verify each one exists in `prep_recipes` with `status = 'published'`.
 *
 * @param components - The plate spec's component groups (JSONB structure)
 * @param excludeSlug - Optional slug to exclude from validation (the plate spec
 *   being published itself, to avoid a false positive).
 * @returns A promise resolving to the validation result.
 */
export async function validatePlateSpecComponentRefs(
  components: PlateComponentGroup[],
  excludeSlug?: string,
): Promise<SubRecipeValidationResult> {
  // 1. Collect every unique prep_recipe_ref slug
  const refSlugs = extractComponentRefSlugs(components);

  // Remove the plate spec's own slug if provided
  if (excludeSlug) {
    refSlugs.delete(excludeSlug);
  }
  ...
  if (error) {
    // If the query itself fails, we cannot guarantee validity.
    // Treat it as a hard failure so the user doesn't silently publish bad data.
    console.error('validatePlateSpecComponentRefs: query failed', error.message);
    return {
      valid: false,
      danglingRefs: slugArray, // assume all are dangling to be safe
    };
...
/**
 * Walk all plate spec component groups/items and collect unique
 * `prep_recipe_ref` values. Skips empty strings and undefined values.
 */
export function extractComponentRefSlugs(
  components: PlateComponentGroup[],
): Set<string> {
  const slugs = new Set<string>();
