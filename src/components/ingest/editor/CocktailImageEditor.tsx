...
export function CocktailImageEditor() {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (url) {
      ...
      dispatch({ type: 'SET_COCKTAIL_IMAGE', payload: url });
  ...
  const handleGenerate = async () => {
    if (!draft.name.trim()) {
      toast({ title: 'Name required', description: 'Add a cocktail name before generating an image' });
      return;
    }
  ...
  const handleRemove = () => {
    dispatch({ type: 'SET_COCKTAIL_IMAGE', payload: null });
    setImageLoaded(false);
  };
