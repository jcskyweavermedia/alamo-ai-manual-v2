...
interface WineAISheetProps {
  wine: Wine;
  action: WineAIAction | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function WineAISheet({ wine, action, open, onOpenChange }: WineAISheetProps) {
  ...
  const actionLabel = WINE_AI_ACTIONS.find(a => a.key === action)?.label ?? '';
  ...
  useEffect(() => {
    ...
    askProduct(actionLabel, {
      ...
      actionOptions: {
        action,
        itemContext: wine as unknown as Record<string, unknown>,
      },
    });
  }, [action, open]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[70vh] flex flex-col rounded-t-xl">
        <SheetHeader className="shrink-0">
          ...
          <SheetDescription>{wine.name}</SheetDescription>
        </SheetHeader>
