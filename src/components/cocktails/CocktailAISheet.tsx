...
import type { CocktailAIAction } from '@/data/mock-cocktails';
import { COCKTAIL_AI_ACTIONS } from '@/data/mock-cocktails';

interface CocktailAISheetProps {
  cocktail: Cocktail;
  action: CocktailAIAction | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function CocktailAISheet({ cocktail, action, open, onOpenChange }: CocktailAISheetProps) {
  ...
  const actionLabel = COCKTAIL_AI_ACTIONS.find(a => a.key === action)?.label ?? '';
  ...
  useEffect(() => {
    ...
    askProduct(actionLabel, {
      ...
      actionOptions: {
        action,
        itemContext: cocktail as unknown as Record<string, unknown>,
      },
    });
  }, [action, open]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[70vh] flex flex-col rounded-t-xl">
        <SheetHeader className="shrink-0">
          ...
          <SheetDescription>{cocktail.name}</SheetDescription>
        </SheetHeader>
          ...
          {action === 'questions' && !isLoading && (
            <div className="flex gap-2 mt-4">
              <Input
                ...
                placeholder="Ask anything about this cocktail..."
